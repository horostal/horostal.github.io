...
再看了别人题解，直接爆破得到密钥是12345，这就是你的512比特私钥吗？出题是一点不长脑子是吧！！！
...
流量包里面除了False还有Data update，有用信息肯定藏在Data update里面；先将数据处理完，不然没有数据写在代码里太过冗长；看0rays题解才发现需要进行异或。
...
那么就是单纯异或操作，异或`\x10`就行；抽象题目，这种题目就应该滚一边去，放密码里就是XD
...
又是XD，将压缩包文件拆成五个部分放在五文件里，逐位提取到同个文件组成压缩包。
...
到这里是比赛时做的，后面用vera怎么挂载都不行，结果等题解出来你告诉我要把`金箍棒.png`作为keyfile，那你写个pass1在这干嘛，出题人是不是左脑缺指令集右脑缺执行库，垃圾题目。

### 总结

决赛题目就是一坨，密码全是屎，还不如初赛题目。
